...
reset: ## DBのリセット
	docker compose -f $(pf) -p $(pn) exec -it scripts npx prisma migrate reset -f
	docker compose -f $(pf) -p $(pn) exec -it scripts npx prisma migrate dev
	docker compose -f $(pf) -p $(pn) exec -it scripts npm run seed -w scripts
...
scripts-shell: ## shellに入る
	docker compose -f $(pf) -p $(pn) exec -it scripts bash
...
check: ## コードフォーマット
# user_front
	docker compose -f $(pf) -p $(pn) exec -it scripts npx prettier . --write
	docker compose -f $(pf) -p $(pn) exec -it scripts npx eslint . --fix
	docker compose -f $(pf) -p $(pn) exec -it scripts npx tsc --noEmit
# user_front
