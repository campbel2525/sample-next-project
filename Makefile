...
reset: ## DBのリセット
	...
	docker compose -f $(pf) -p $(pn) exec -it migration npm run seed
...
install: ## インストール
	...
	docker compose -f $(pf) -p $(pn) exec -it user-front npm install
	docker compose -f $(pf) -p $(pn) exec -it user-front npx prisma generate
...
# user-front
	...
	docker compose -f $(pf) -p $(pn) exec -it user-front npm run lint -- --fix
